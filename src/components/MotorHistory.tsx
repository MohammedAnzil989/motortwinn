// app/components/MotorHistory.tsx
"use client";
...
export default function MotorHistory() {
  ...
  const getStatusColor = (action: 'start' | 'stop') => {
    return action === 'start' ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50';
  };
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-semibold text-lg text-gray-700">Motor Operation History</h3>
          ...
          {motorHistory.slice().reverse().map((record) => (
            <div
              ...
              className={`p-3 rounded-lg border ${
                record.action === 'start' ? 'border-green-200' : 'border-red-200'
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <span className="text-lg">{getStatusIcon(record.action)}</span>
                  <div>
                    <span className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(record.action)}`}>
